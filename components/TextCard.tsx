        ...
        if (files.length > 0) { 
            ...
            if (newFiles && newFiles.length > 1) {
                ...
                if (arrFiles.length >= fileLimit) {
                    setFileErrorMsg(`Please choose either 1 video or up to ${fileLimit} images.`)
                ...
                if (file && file.length > 0) {
                    ...
                    // check if all old files are images
            ...
            if (files && files.length > 1) {
                ...
                const images = fileTypes.filter((type: string) => type.includes("image"))
                if (files.length >= fileLimit) {
                    setError(true, `Please choose either 1 video or up to ${fileLimit} images`)
                    setErrorTimeout(5000)
                } else if (videos) {
                    setError(true, `Please choose either 1 video or up to ${fileLimit} images`)
                    setErrorTimeout(5000)
        ...
        <div className="">
                ...
                postTextBoxShown ? 
                    <Modal
                        ...
                        styles={{
                            ...
                            modal: {
                                content: {
                                    class: 'modal_main_content max-w-xl w-[100%] mt-20 mb-20 screen-sm:pt-0 screen-sm:mt-5 screen-sm: screen-sm:top-0 screen-sm:left-0 screen-sm:right-0 screen-sm:text-center screen-sm:ml-auto screen-sm:mr-auto screen-sm:w-full overflow-auto',
                                    position: 'fixed',
                                    border: 'border-b border-gray-100 dark:border-borderDarkMode dark:border-b-gray-50 dark:border-opacity-10'
                                },
                                height: 'h-auto'
                            },
                        ...
                        showCloseIcon={true}
                    >
                        <div className={setClass(" mt-6 screen-sm:p-4 screen-sm:pb-1")}>
...

            {
                isFileError && <Transition
                    transition={'pop'}
                    className={'media_error_upload fixed bottom-6 right-0 left-0 z-50 aniamte-pop max-w-[24rem] mr-auto ml-auto text-center'}
                >
                    <div className="bg-primary p-[0.6rem] w-auto inline-block transition transition-scale rounded"
                    >
                       {fileErrorMsg}
                    </div>
                </Transition>
